...
module.exports = async (req, res) => {
    try {
        ...
        if (response.status === 200) {
            ...
            res.status(200).json({ error: false, message: "OTP sent successfully", data: isReqStored.otpid });
        } else {
            res.status(response.status).json({ error: true, message: "Failed to send OTP" });
        }
    } catch (error) {
